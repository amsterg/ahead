...
class MDN(nn.Module):
    def __init__(self,
        ...
        self.fc1 = nn.Linear(20 * np.prod(self.lin_in_shape),
                             num_gaussians * 12)
        ...
        if load_model:
            model_pickle = torch.load(self.config_yml['MODEL_SAVE_DIR'] +
    ...
    def gaussian_distribution(self, y, mu, sigma, rho=0):
        ...
        assert sigma_x.shape == sigma_y.shape, print(sigma_x.shape,
                                                     sigma_y.shape)
...
def infer(net, epoch, x_var):
    model_pickle = torch.load(net.config_yml['MODEL_SAVE_DIR'] +
                              'Epoch_{}.pt'.format(epoch))
